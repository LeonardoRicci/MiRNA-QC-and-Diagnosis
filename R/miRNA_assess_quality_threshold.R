...
#' @export
miRNA_assessQualityThreshold <- function(inputDataset, significanceLevel=0.05, outputFileName="", sep='\t') {

	cat("\nLOG: miRNA_assessQualityThreshold() called.\n")
	...
	if (length(inputDataset[1,]) > 7) {
		cat("WARNING: more than 7 dataset columns. Columns other than 'Subject', 'miRNA', 'Mean', 'StdDev', 'SampleSize', 'Class' will be ignored.\n")
	}
	...
	cat("LOG:\tQuality threshold values written to ", outputFileName, " successfully.\n", sep="")

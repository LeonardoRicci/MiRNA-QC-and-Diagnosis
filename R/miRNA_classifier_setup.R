...
#' @export
miRNA_classifierSetup <- function(inputDataset, inputTargetList, inputVersusList=character(), inputMiRNAList=character(), coeffList=double(), outputFileBasename="", sep='\t', plotFormat="pdf") {

	cat("\nLOG: miRNA_classifierSetup() called.\n")
	...
	if (!(("Subject" %in% colnames(inputDataset)) & ("miRNA" %in% colnames(inputDataset)) & ("Mean" %in% colnames(inputDataset)) & ("StdDev" %in% colnames(inputDataset)) & ("SampleSize" %in% colnames(inputDataset) & ("Class" %in% colnames(inputDataset)))))  {
		cat("ERROR: unsuitable dataset format. Dataset must contain columns 'Subject', 'miRNA', 'Mean', 'StdDev', 'SampleSize', 'Class'.\n")
		return(NULL)
	}
	...
	if ((length(inputMiRNAList) == 1) && (length(coeffList) == 0)) {
		...
		normalizerFlag <- 1
		classifierFlag <- 0
		cat("LOG:\tmiRNA_classifierSetup() is running in Analysis mode (with normalizer).\n")
	} else if ((length(inputMiRNAList) == 0) && (length(coeffList) == 0)) {
		normalizerFlag <- 0
		classifierFlag <- 0
		cat("LOG:\tmiRNA_classifierSetup() is running in Analysis mode (without normalizer).\n")
	} else if (length(inputMiRNAList) == length(coeffList)) {
		normalizerFlag <- 0
		classifierFlag <- 1
		cat("LOG:\tmiRNA_classifierSetup() is running in Training mode.\n")
	} else {
	...
	for (currentFeature in availableFeatures) {
		...
		if (!classifierFlag) {
			if (length(completeDataFrame$Score[completeDataFrame$Diagnosis == "target"]) > 2) {
				...
				cat("WARNING: miRNA '", currentFeature, "' has less than 3 Subjects in the target set. Cannot carry out Shapiro-Wilk test.\n")
			}
		}
		# Versus
		...
		if (!classifierFlag) {
			if (length(completeDataFrame$Score[completeDataFrame$Diagnosis == "versus"]) > 2) {
				...
				cat("WARNING: miRNA '", currentFeature, "' has less than 3 Subjects in the versus set. Cannot carry out Shapiro-Wilk test.\n")
			}
		}
		# Target and Versus
		...
		if (!classifierFlag) {
			if (length(completeDataFrame$Score) > 2) {
				...
				shapiroWilkVersus <- NA
				cat("WARNING: miRNA '", currentFeature, "' has less than 3 Subjects in the dataset. Cannot carry out Shapiro-Wilk test.\n")
			}
		}
		localPrecision <- 1
		...
		if (classifierFlag) {
			...
			cat("LOG:\tDiagnostic thresholds data frame written to ", thresholdFileName, " successfully.\n", sep="")
	...
	if (!classifierFlag) {
		...
		cat("LOG:\tmiRNA statistics and correlation matrices written to ", statisticsFileName, " successfully.\n", sep="")
	}
